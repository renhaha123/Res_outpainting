...
class Places2(Dataset):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.img_file = sorted(glob.glob('{:s}/*'.format(opt.img_file)))
    ...
    def __getitem__(self, item):
        img = Image.open(self.img_file[item]).convert('RGB')
        ...
        if self.opt.mask_file:
            ...
            mask = load_center_mask(img)

        img_name = os.path.basename(self.img_file[item])

        return img, rtv_img, mask, img_name
